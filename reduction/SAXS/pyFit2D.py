...
class AgBehCalib(object):
    ...
    def __init__(self,fit2Dexe,file,SDD,points=None,macro=None):
    ...
    def showPoints(self):
        im = Image.open(r'E:\\Vaxster\\181119_ILLProben\\images\\im_0066693_craw.tiff')
        image = np.flipud(np.array(im))
        self.ax.imshow(np.log10(image)) 
...
#        plt.draw()
    ...
    def compute(self):
        p = subprocess.Popen([self.fit2Dexe,"-com"],stdin=subprocess.PIPE,stdout=subprocess.PIPE,universal_newlines=True,shell=False)
        parsed_string = ''
        ...
        parsed_string+='EXIT\n'
        stdout= p.command(parsed_string)[0]
        #print(stdout.split('\n')[165:])
        f = open('macro'+str(self.SDD)+'.my','w')
...
class ReduceDataWBeamCenter(object):
    ...
    def __init__(self,fit2Dexe,InDir,OutDir,List,ListTrans,Mask,Calib,prefix = 'im_00', suffix = '_craw.tiff',compute=True):
        self.logFile = str(np.datetime64('today','D'))+'.log'
        self.InDir = InDir
        self.OutDir = OutDir
        self.List = List
    ...
    def compute(self):
        for i,iData in enumerate(self.List):
            ...
            parsed_string = ''
            parsed_string+='SAXS / GISAXS\nINPUT\n'+self.InDir+self.prefix+str(self.ListTrans[i])+self.suffix+'\n'
...
class GetStatData(object):
    ...
    def compute(self):
        for iData in self.List:
            ...
            p = subprocess.Popen([self.fit2Dexe,"-com"],stdin=subprocess.PIPE,stdout=subprocess.PIPE,universal_newlines=True,shell=True,creationflags=subprocess.DETACHED_PROCESS)
            parsed_string = ''
            parsed_string+='SAXS / GISAXS\nINPUT\n'+self.InDir+self.prefix+str(iData)+self.suffix+'\n'
